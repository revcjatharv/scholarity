...
router.post('/', async (req, res, next) => {
    ...
    const { testType, userId } = req === null || req === void 0 ? void 0 : req.body;
    console.log(date, time, testType);
    let testList = await test_list_model_1.TestList.find({ date: { $gte: date }, testType, isConducted: false });
    let resultsFromUserEnroll = await user_test_mapping_model_1.UserTest
        .find({ userId }).populate('userId').populate('testId');
    const newtestList = [];
    for (let i = 0; i < testList.length; i++) {
        const element = testList[i];
        const isPresent = resultsFromUserEnroll.find((x) => x && x.testId && x.testId.id === element.id);
        const newElem = Object.assign({}, element._doc);
...
router.post('/testList', (req, res, next) => {
    ...
    const { date = new Date(), isConducted = false, isTestStarted = false, testName = '', testDescription = '', testType = '', testTime = '', timer = 20000, maxPrize = 0, minPrize = 0, totalQuestions = 0, entryFee = 0, instruction = '' } = req === null || req === void 0 ? void 0 : req.body;
...
router.post('/getFreetest', async (req, res, next) => {
    const list = [{
            "timer": 25.0,
